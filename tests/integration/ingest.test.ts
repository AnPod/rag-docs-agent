...
describe("POST /api/ingest", () => {
  ...
  describe("Validation", () => {
    ...
    it("should return 400 for unsupported file types (should reject non .md/.txt)", async () => {
      ...
      vi.mocked(extractTextFromFile).mockResolvedValue({
        content: "content",
        metadata: { source: "test.js", lineCount: 1 },
      });
    ...
    it("should accept .md files", async () => {
      ...
      vi.mocked(extractTextFromFile).mockResolvedValue({
        ...
        metadata: { source: "test.md", lineCount: 1 },
      });
      vi.mocked(chunkText).mockReturnValue([
        {
          id: "chunk-0",
          content: "test content",
          metadata: { source: "test.md", startLine: 1, endLine: 1 },
        },
      ]);
    ...
    it("should accept .txt files", async () => {
      ...
      const { extractTextFromFile, chunkText, generateEmbeddings } =
        await import("@/lib");

      vi.mocked(extractTextFromFile).mockResolvedValue({
        content: "test content",
        metadata: { source: "test.txt", lineCount: 1 },
      });
      vi.mocked(chunkText).mockReturnValue([
        {
          id: "chunk-0",
          content: "test content",
          metadata: { source: "test.txt", startLine: 1, endLine: 1 },
        },
      ]);
      vi.mocked(generateEmbeddings).mockResolvedValue([[1, 2, 3]]);
  ...
  describe("Edge cases", () => {
    it("should handle filenames with special characters", async () => {
      const mockFile = new File(["content"], " файл с пробелами.md", {
        type: "text/markdown",
      });
      const FormDataMock = {
        get: vi.fn().mockReturnValue(mockFile),
      ...
      vi.mocked(extractTextFromFile).mockResolvedValue({
        content: "test content",
        metadata: { source: " файл с пробелами.md", lineCount: 1 },
      });
      vi.mocked(chunkText).mockReturnValue([
        {
          id: "chunk-0",
    ...
    it("should handle empty files gracefully", async () => {
      ...
      vi.mocked(extractTextFromFile).mockResolvedValue({
        content: "",
        metadata: { source: "empty.md", lineCount: 0 },
      });
      vi.mocked(chunkText).mockReturnValue([]);
